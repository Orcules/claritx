...
async def run_batch_job_flow(
    ...
    logger.info(f"Starting Batch Job Flow... Type: {asset_type}, Limit: {limit}, Provider: {provider}, Source: {session_source}, Auto-Process: {auto_process}")
    ...
    try:
        async with engine.connect() as _conn:
            ...
            if today_count == 0:
                logger.info("Screener cache not updated today — refreshing from FMP...")
            ...
            logger.info(f"Screener cache has {today_count} symbols updated today")
            ...
            logger.info(f"Selected {len(top_candidates)} {asset_type} candidates from screener_cache")

    except Exception as db_e:
        logger.error(f"Screener cache query failed, falling back to FMP API: {db_e}")
        import traceback
        traceback.print_exc()
    ...
    logger.info(f"Final candidate count: {len(top_candidates)} top {asset_type} candidates.")

    # 3. Process Candidates
    if provider == 'kie_direct':
        queue_url = os.environ.get("BATCH_QUEUE_URL") or os.environ.get("SQS_QUEUE_URL")
        if queue_url:
            logger.info(f"Routing {len(top_candidates)} candidates to Kie Direct via distributed SQS messages...")
            import boto3
            ...
            for candidate in top_candidates:
                try:
                    ...
                    logger.error(f"Failed to queue single ticker for {candidate.get('symbol')}: {e}")
        ...
        queue_url = os.environ.get("BATCH_QUEUE_URL") or os.environ.get("SQS_QUEUE_URL")
        if queue_url:
            logger.info(f"Routing {len(top_candidates)} candidates to Google Batch SQS Staging...")
            import boto3
            import uuid
            ...
            try:
                ...
                logger.error(f"Failed to create master batch record: {e}")
            ...
            for candidate in top_candidates:
                try:
                    ...
                    logger.error(f"Failed to queue staging ticker for {candidate.get('symbol')}: {e}")
    ...
    if provider == 'google_batch':
        logger.warning("Notice: Auto-polling is disabled for Google Batch to prevent execution timeout. Job will submit and exit cleanly for manual checking.")
        auto_process = False

    if not auto_process:
        logger.warning(f"Batch {batch_id} submitted. Auto-processing disabled. Returning immediately.")
        return {
            "status": "success",
            "universe_size": len(top_candidates),
            "selected_count": len(top_candidates),
    ...
    logger.info(f"Batch {batch_id} submitted. Starting polling loop...")
    ...
    for i in range(max_retries):
        try:
            logger.info(f"Polling Batch {batch_id} (Attempt {i+1}/{max_retries})...")
            ...
            if status == "processed":
                logger.info(f"Batch {batch_id} complete and processed!")
                return {
                    "status": "success",
                ...
                return {"status": "error", "message": f"Batch Job Failed: {poll_result.get('message')}"}
                 ...
                 logger.warning(f"Batch Status: {status}. Waiting...")
    ...
    if creds_json:
        ...
        if not creds_json.startswith("{"):
            try:
                ...
                logger.error(f"Failed to base64 decode credentials: {e}")
        ...
        for p in possible_paths:
            if os.path.exists(p):
                ...
                logger.info(f"[Batch] Using GCP credentials from file: {p}")
                return p
    ...
    try:
        ...
        if conn_url:
            try:
                ...
                logger.error(f"Warning: could not fetch current batch status from DB: {e}")

        job = client.batches.get(name=batch_id)
        logger.info(f"Batch Job {batch_id} Status from Vertex: {job.state.name} (DB status: {current_status})")
        ...
        try:
            ...
            if job.state.name == "JOB_STATE_FAILED":
                 ...
                 logger.error(f"Batch Job Failed Details: {job}")
            ...
            if conn_url:
                with psycopg2.connect(conn_url) as conn:
                    ...
                    conn.commit()
                    logger.info(f"Updated batch job {batch_id} status to {job.state.name}")
        except Exception as db_e:
            logger.error(f"Failed to update batch job status in DB: {db_e}")
            error_msg = ""
        ...
        logger.info(f"Job Complete. Output Location: {output_uri}")
        ...
        logger.info(f"Found {len(jsonl_blobs)} result files.")
            ...
            with psycopg2.connect(conn_url) as persistent_conn:
                with persistent_conn.cursor() as cur:
                    ...
                    for blob in jsonl_blobs:
                        logger.info(f"Streaming and processing results from {blob.name}...")
                        ...
                        with blob.open("r") as f:
                            for line in f:
                                ...
                                try:
                                    line_count += 1
                                    if line_count % 50 == 0:
                                        logger.info(f"Progress: Read {line_count} lines from GCS...")
                                    ...
                                    try:
                                        ...
                                        try:
                                            ...
                                            if "Extra data" in str(jde):
                                                ...
                                                logger.info(f"Fixed extra trailing data issue for {symbol} (char {jde.pos})")
                                            else:
                                                raise
                                    except Exception as parse_e:
                                        logger.error(f"Failed to parse analysis content for {symbol}: {parse_e}")
                                        continue
                                    ...
                                    try:
                                        ...
                                        if request_obj:
                                            ...
                                            if "DATA CONTEXT START:" in prompt_text:
                                                ...
                                                try:
                                                    ...
                                                    logger.error(f"Could not JSON-parse embedded financial context for {symbol}: {parse_fin_e}")
                                        ...
                                        logger.info(f"Saved full analysis for {symbol} to stock_analyses.")
                                        result_count += 1
                                            
                                    except Exception as save_e:
                                        logger.error(f"Failed to reconstruct/save analysis for {symbol}: {save_e}")
                                
                                except Exception as line_e:
                                    logger.error(f"Error processing line: {line_e}")
                                    continue
...
async def submit_google_batch_job_from_staged_prompts(batch_id: str, prompts: List[str]):
    ...
    try:
        ...
        if not bucket_name:
            logger.warning("Missing GCS_BUCKET_NAME or GOOGLE_CLOUD_PROJECT to infer bucket.")
            return
        ...
        logger.info(f"Uploading batch file to gs://{bucket_name}/{blob_name}")
        ...
        # 2. Submit Batch Job
        logger.info(f"Submitting batch job {batch_id} to Vertex AI...")
        ...
        try:
            async with engine.connect() as _conn:
                ...
                await _conn.commit()
                logger.info(f"Batch Job {batch_job.name} successfully submitted and staged data cleaned.")
        except Exception as e:
            logger.error(f"Failed to update batch status in DB: {e}")

    except Exception as e:
        logger.error(f"Failed to submit Vertex job for {batch_id}: {e}")
...
async def gather_single_google_batch_candidate(candidate: Dict[str, str], batch_id: str, target_limit: int, conn=None) -> Dict[str, Any]:
    ...
    logger.info(f"Gathering Google Batch Context for {symbol} (Batch {batch_id[:15]}...)")
    import uuid
    import time
    from sqlalchemy import text
    ...
    try:
        ...
        try:
            async with engine.connect() as _conn:
                ...
                if prev_row:
                    ...
                    logger.info(f"Found previous analysis for {symbol} for comparison in batch gathering.")
        except Exception as e:
            logger.error(f"Failed to fetch previous analysis for {symbol} in batch gathering: {e}")
        ...
        logger.error(f"Gathering data failed for {symbol}: {e}")
    ...
    try:
        ...
        logger.info(f"Batch {batch_id[:10]} Progress: {count}/{target_limit} collected.")

        if count >= target_limit:
            logger.info(f"Batch {batch_id[:10]} complete! Submitting master Vertex job...")
            async with engine.connect() as _conn:
        ...
        logger.error(f"CRITICAL: Failed to save Google Batch prompt for {symbol} to Postgres DB: {e}")
        return {"error": str(e)}
...
async def process_single_kie_candidate(candidate: Dict[str, str], conn=None, asset_type: str = 'stock') -> Dict[str, Any]:
    ...
    logger.info(f"Starting Single Kie.ai Processing for {symbol}...")
    ...
    try:
        async with engine.connect() as _conn:
            ...
            if prev_row:
                previous_analysis_data = prev_row[0]
                logger.info(f"Found previous analysis for {symbol} for comparison in single kie processing.")
    except Exception as e:
        logger.error(f"Failed to fetch previous analysis for {symbol} in single kie processing: {e}")
    ...
    if isinstance(fmp_data, Exception):
        logger.error(f"FMP fetch failed for {symbol}: {fmp_data}")
        fmp_data = None
...
async def reconcile_stuck_staging_jobs(conn) -> Dict[str, Any]:
    ...
    """
    logger.warning("Running Batch Reconciliation for stuck 'staging' jobs...")
    try:
        ...
        if not stuck_jobs:
            logger.warning("No stuck staging jobs found.")
            return {"status": "success", "reconciled": 0}
            
        reconciled_count = 0
        for job_id, target_count, item_count in stuck_jobs:
            logger.warning(f"Checking stuck job {job_id} (target: {target_count}, current: {item_count})")
            ...
            if target_count > 0 and actual_count >= target_count:
                logger.info(f"Job {job_id} has reached target {target_count}. Triggering submission...")
                should_submit = True
                ...
                logger.warning(f"Job {job_id} is stalled at {actual_count} items. Checking if we should force submit...")
                should_submit = True # For now, if it's > 30m old and stable, submit it.
            
            if should_submit:
                logger.info(f"Reconciling job {job_id}: Fetching {actual_count} prompts...")
                ...
                if all_prompts:
                    ...
                    reconciled_count += 1
                    logger.info(f"Reconciled and submitted job {job_id} with {len(all_prompts)} items.")
                else:
                    logger.error(f"Job {job_id} had no prompts in DB. Marking as error.")
        ...
        try:
            logger.warning("Vacuuming orphaned prompts...")
            await conn.execute(text("""
