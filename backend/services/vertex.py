...
async def analyze_stock_with_vertex(symbol: str, financial_data: Dict[str, Any], conn=None, previous_analysis: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    if previous_analysis is None and conn:
        try:
            ...
            if prev_row:
                ...
                logger.info(f"[Vertex] Found previous analysis for {symbol} for comparison.")
        except Exception as e:
            logger.error(f"Failed to fetch previous analysis in vertex service: {e}")
    ...
    try:
        ...
        if creds_json:
            ...
            for p in possible_paths:
                if os.path.exists(p):
                    ...
                    logger.info(f"[Vertex] Using GCP credentials from file: {p}")
                    break
        ...
        logger.error(f"Vertex Analysis Failed: {e}")
...
async def upload_to_gcs(data: bytes, filename: str, content_type: str = "image/jpeg") -> str:
    ...
    if not bucket_name:
        logger.warning("[GCS] No bucket name configured, skipping upload.")
        return ""
    ...
    try:
        ...
        try:
            ...
            logger.error(f"[GCS] Could not make public (policy may prevent it): {e}")
...
async def generate_image_with_vertex(prompt: str, slug: str) -> str:
    ...
    if not cred_path:
        logger.warning("[Chat] Warning: GCP credentials not found, trying ADC")
    ...
    try:
        ...
        logger.info(f"[Vertex] Generating image for prompt: {styled_prompt[:80]}...")
        response = client.models.generate_content(
