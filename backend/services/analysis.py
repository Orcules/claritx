...
from services.gemini import analyze_stock_with_gemini
...
async def stock_analysis_internal(symbol: str, source: str = "api", conn=None) -> Dict[str, Any]:
    ...
    """
    symbol = symbol.upper()
    print(f"[{source}] Starting analysis for {symbol}")
    ...
    try:
        async with _engine.connect() as _conn:
            ...
            if prev_row:
                ...
                print(f"[{source}] Found previous analysis for {symbol} for comparison.")
    except Exception as e:
        print(f"Failed to fetch previous analysis for {symbol}: {e}")
