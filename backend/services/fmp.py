...
async def acquire_distributed_fmp_token():
    ...
    if not engine:
        try:
            ...
            print(f"CRITICAL: Failed to initialize DB engine for Rate Limiter: {e}")
    ...
    if not engine:
        print("CRITICAL: DB engine still None after lazy-init attempt. Falling back to local limits!")
        await fmp_limiter.acquire()
        return
    ...
    while True:
        try:
            ...
            print(f"CRITICAL: DB Rate Limiter Transaction Failed! Error: {e}")
            # Fallback to pure local limiting if DB panics during transaction
            await fmp_limiter.acquire()
            return
            
        if sleep_time > 0:
            print(f"FMP GLOBAL RATE LIMIT REACHED (Count: {count}). Sleeping Lambda for {sleep_time:.2f} seconds...")
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            if profile_resp.status_code == 200:
                ...
                print(f"FMP profile failed: {profile_resp.status_code}")
                results["profile"] = {}
        except Exception as e:
            print(f"FMP profile error: {e}. Raising exception to trigger retry.")
            raise e
        ...
        for key, url in endpoints.items():
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    print(f"FMP {key} failed: {resp.status_code}")
                    results[key] = None
            except Exception as e:
                print(f"FMP {key} error: {e}. Raising exception to trigger retry.")
                raise e
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f"FMP Screener failed: {resp.status_code}")
                return []
        except Exception as e:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f"FMP Active List failed: {resp.status_code}")
                return []
        except Exception as e:
            print(f"FMP Active List error: {e}")
            return []
