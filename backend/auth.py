...
async def verify_token(authorization: str = Header(None)):
    ...
    if not user_pool_id:
        # Fallback if env vars not set (e.g. local test)
        return token
    ...
    try:
        ...
        if claims.get('client_id') != app_client_id and claims.get('aud') != app_client_id:
                print(f"Warning: Token client_id {claims.get('client_id')} != {app_client_id}")
...
async def verify_admin(claims: dict = Depends(verify_token)):
    ...
    if 'Admins' not in groups:
        print(f"User {claims.get('sub')} attempted admin action but lacks Admins group. Groups: {groups}")
