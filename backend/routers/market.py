...
def _dispatch_background_scan(background_tasks, job_id: str = "bg-scan"):
    ...
    if queue_url:
        try:
            ...
            logger.info(f"[Market Scan] Dispatched to SQS BackgroundTasksQueue (job_id={job_id})")
        except Exception as e:
            logger.error(f"[Market Scan] SQS dispatch failed ({e}), falling back to local background task")
            background_tasks.add_task(scan_market_opportunities, job_id)
    else:
        # Local dev: run as FastAPI background task
        background_tasks.add_task(scan_market_opportunities, job_id)
        logger.info(f"[Market Scan] Dispatched as local background task (no SQS_BACKGROUND_QUEUE_URL set)")
...
{{
  ...
  "explanation": "Brief explanation"
    ...
    try:
        ...
        logger.error(f"AI explanation failed for {symbol}: {e}")
        return {
...
async def refresh_market_movers() -> dict:
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            logger.error(f"[Market Refresh] Batch fetch failed: {e}")
    ...
    try:
        async for conn in get_db_connection():
            ...
            await conn.commit()
            break
        logger.info(f"[Market Refresh] Cache updated — {len(movers)} movers")
    except Exception as e:
        logger.error(f"[Market Refresh] Cache write failed: {e}")
...
def _dispatch_market_refresh():
    ...
    queue_url = os.environ.get("SQS_BACKGROUND_QUEUE_URL")
    if not queue_url:
        logger.warning("[Market Movers] No SQS queue configured — skipping background refresh")
        return
    try:
        sqs = boto3.client("sqs", region_name=os.environ.get("AWS_DEFAULT_REGION", "us-east-1"))
        sqs.send_message(QueueUrl=queue_url, MessageBody=json.dumps({"action": "REFRESH_MARKET_MOVERS"}))
        logger.info("[Market Movers] Background refresh dispatched to SQS")
    except Exception as e:
        logger.error(f"[Market Movers] SQS dispatch failed: {e}")
...
async def _get_market_movers_cached_response(response: Response):
    ...
    try:
        ...
        logger.error(f"[Market Movers] Cache read error: {e}")
...
async def scan_market_opportunities(job_id: str = "bg-scan"):
    ...
    if _scan_in_progress:
        logger.warning(f"[Market Scan] Scan already in progress. Skipping duplicate job '{job_id}'.")
        return
    _scan_in_progress = True
    logger.info(f"[Market Scan] Starting scan job '{job_id}'")
    try:
...
[
  {{
    ...
    "timeframe": "short-term"
        ...
        logger.info(f"Found {len(valid_opps)} market opportunities via Vertex.")
        ...
        logger.error(f"Market Ops Error: {err_msg}")
        logger.info(f"Traceback: {tb_str}")
        ...
        try:
            ...
            logger.error(f"Failed to save error log for market ops: {log_err}")
    finally:
        _scan_in_progress = False
        logger.info(f"[Market Scan] Scan job '{job_id}' finished, lock released.")
...
async def get_market_opportunities(background_tasks: BackgroundTasks, refresh: bool = False):
    async for conn in get_db_connection():
        ...
        if not _scan_in_progress:
            ...
            logger.info("[Market Scan] Scan already in progress - not queuing another.")
