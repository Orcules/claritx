...
async def _deduct_credits(conn, user_id: str, cost: int, action: str):
    """Credit check disabled — all users permitted."""
    return
...
async def stock_analysis_with_cache(req: StockAnalysisRequest, token: Optional[Dict] = Depends(optional_token), conn = Depends(get_db_connection)):
    ...
    await _deduct_credits(conn, user_id, 1, "analyze-stock")
