...
async def stripe_webhook(request: Request):
    ...
    if not sig_header or not webhook_secret:
        logger.error("Webhook Error: Missing signature or webhook secret config")
        raise HTTPException(status_code=400, detail="Missing signature or webhook secret config")

    try:
        event = stripe.Webhook.construct_event(payload, sig_header, webhook_secret)
        logger.info(f">>> STRIPE EVENT RECEIVED: {event['type']} (ID: {event['id']})")
    except ValueError:
        logger.info("Invalid payload in stripe webhook")
        ...
        if user_id:
            async for conn in get_db_connection():
                try:
                    ...
                    await conn.commit()
                    logger.info(f"Successfully processed {event['type']} for user {user_id}")
                except Exception as db_err:
                    logger.error(f"Error updating DB in stripe webhook: {db_err}")
                finally:
                    break
    ...
    # ── customer.subscription.deleted ──────────────────────────────────────────
        ...
        async for conn in get_db_connection():
            try:
                ...
                await conn.commit()
                logger.info(f"User downgraded to free for subscription {sub_id}")
            except Exception as db_err:
                logger.error(f"Error handling subscription.deleted: {db_err}")
            finally:
                break
    ...
    # ── customer.subscription.updated ──────────────────────────────────────────
        ...
        logger.info(f"Subscription updated: {sub_id}, status={status}, cancel_at_period_end={cancel_at_period_end}")
        ...
        async for conn in get_db_connection():
            try:
                ...
                await conn.commit()
                logger.info(f"Subscription updated to {status} for {sub_id}")
            except Exception as db_err:
                logger.error(f"Error handling subscription.updated: {db_err}")
            finally:
                break
    ...
    # ── invoice.payment_failed ──────────────────────────────────────────────────
        ...
        logger.error(f"Payment failed for customer {customer_id}, subscription {sub_id}, attempt {attempt_count}")
        ...
        if sub_id:
            async for conn in get_db_connection():
                try:
                    ...
                    logger.error(f"Error handling invoice.payment_failed: {db_err}")
                finally:
                    break
    ...
    # ── invoice.paid ────────────────────────────────────────────────────────────
        ...
        logger.info(f"Invoice paid for customer {customer_id}, subscription {sub_id}")
        ...
        if sub_id:
            async for conn in get_db_connection():
                try:
                    ...
                    await conn.commit()
                    logger.info(f"Credits reset for new billing period, sub {sub_id}")
                except Exception as db_err:
                    logger.error(f"Error handling invoice.paid: {db_err}")
                finally:
                    break
