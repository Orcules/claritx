...
async def get_blogs(limit: int = 200, offset: int = 0):
    ...
    try:
        async for conn in get_db_connection():
            ...
            bucket_domain = f"https://storage.googleapis.com/{os.environ.get('GCS_BUCKET_NAME', 'creative-project-batch-files')}/"
            processed = []
            for row in rows:
                d = dict(row)
                img = d.get("image_url")
...
async def proxy_image(path: str):
    ...
    bucket_name = os.environ.get("GCS_BUCKET_NAME", "creative-project-batch-files")
